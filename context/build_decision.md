...
- Input artifacts: [Agentic MVP](agentic_mvp.md) v1.2; [War game](war_game.md) v2.1; [Metrics](metrics_learning.md) v1.1; [GTM](gtm_scenario.md) v1.0; [Product Vision](product_vision.md) v0.14; [Todo before MVP](todo_before_mvp.md); internal SE-DIFM strategy corpus (Notion/Drive, fetched 2026-07-21); Requirements Spec – SE DIFM: gfr.ai, FinAPI & DATEV (EN) v1.0.
...
| Trust & control | Sources, checks, blockers, pause/cancel/return/take-over, durable audit package, four-eyes routing to the Steuerberater. | Human authority is absolute at the Freigabe. "Ready" in the workbench is explicitly **not** "filed". |
| One front door, real agents | Tax Advisor Agent leads; DataJanitor Pro / FristenGuard / AdvisorLens / Document Review Agent visible behind it in one Plan and one Trace. | Each agent has a scope, capability limit, stop rule, and human gate. Named roles are targets, proven against the job. |

### Architecture principle (the vendor-agnostic contract)

- Every external system is reached through an **adapter interface**; no vendor type leaks into the core. Swapping the bookkeeping engine, bank provider, or filing route is an adapter change, not a rewrite.
...
- No re-implementation of bookkeeping computation the engine already performs (we govern and verify it; we don't rebuild it).
- No DATEV write automation; the downstream handoff is human-operated and recorded (simulated in replay).
- No client chat or AI-assisted client-message loop (deferred; human-Send rule preserved for later).
- No real client data (synthetic replay first).
- No Kleinunternehmer §19, e-commerce, Kapitalgesellschaft, employees/payroll, cash business, or Bilanzierungspflicht cases — hard-excluded, fail-closed.
- No structured e-invoice (ZUGFeRD/XRechnung) yet — parse-or-stop, flagged as a near-term gap.
- No Opportunity Radar beyond a single shadow-only overdue-receivables card (delighter, never changes readiness).

---

## 3. The decisions we took, and why

Decisions inherited from the MVP (D001–D022) stand where consistent with reality. The war-game rerun added D008–D014, which record the corrections. This is the consolidated, current set.

| Decision | What we decided | Why (rationale we can defend) |
| --- | --- | --- |
| **DEC-01** (war-game D014) | The MVP is **vendor-agnostic**, built against clean API abstractions. GFR/FinAPI/DATEV are reference integrations behind adapter interfaces, not a lock-in. The greenfield-vs-brownfield framing is rejected. | Naming a vendor in strategy is not the same as coupling to it. A clean adapter architecture lets us integrate the committed systems now and swap them later, and it keeps the vendor-neutral handoff (MET-D009) as an achievable design intent rather than a contradiction. |
| **DEC-02** (war-game D011) | The execution layer **governs and verifies the bookkeeping engine's output through its API** and assembles an evidence-linked review package; it does not re-implement bookkeeping computation. | The engine already does OCR / booking / VAT / reconciliation / §14 validation. Re-deriving that adds a hallucination surface and no verifiable upside. The honest, valuable job is governance, verification, source-linking, and routing — which is engine-agnostic. |
| **DEC-03** (war-game D013) | The current PoC build (`taxfix_harness` codebase and its model-provider config) is **out of the decision matrix**. | We build the MVP from the WoW docs and company strategy, not from the current prototype. Judging the plan by the prototype's model provider or seeded fixtures repeats the "current build" mistake v1.0 already ruled out (D004). |
| **DEC-04** | Bookkeeping dates and work state come from the **integrated systems via their adapters**; DATEV is integrated through its **sanctioned API/partner route** when access is granted, with dates sourced from other systems until then. | DATEV access is partner-gated (AGB 2.6, no OAuth client_credentials, 2FA). This is an integration-sequencing reality, handled by the adapter and a dated onboarding item — not a reason to claim or deny a "DATEV connection". |
| **DEC-05** | The first case is the **one frozen Buchungsprofil** (§2), with hard exclusions enforced by a **fail-closed scope gate** upstream of preparation. | Notion fixes exactly one pilot profile and an explicit exclusion list; without an upstream gate, out-of-scope work (e.g. §19 KU, webshop) can silently pass and certify a wrong result (war-game F030). |
...
| **DEC-07** | Human authority is absolute at the **Steuerberater Freigabe**; "ready" ≠ "filed"; autonomy stays **L2-Prepare** (no auto-submit). | Four-eyes Freigabe is legally mandatory; the Steuerberater carries full StBerG liability. The control layer cannot confirm an ELSTER submission, so it must never claim a filing happened. |
| **DEC-08** | Gate stays **hold → narrow**. There are **no P0 blockers to the synthetic-replay build**; we hold to close the design requirements and the existing G001 open questions. | The rerun's provisional P0s were overruled (DEC-01, DEC-03) or are controlled-live/real-data prerequisites (§6). The remaining work is design requirements and the already-known G001 inputs. |
| **DEC-09** | Metrics are **bound to reality**: ≥1 gate KPI tied to a tracked live metric (FL/rework or prep-time); match denominator pinned to all in-period transactions; cost-per-case baseline established and CP1 gated on margin; a post-submission error-detection-latency KPI bounded to the §347 AO 1-month Einspruchsfrist. | G002 KPIs currently map to no live North Star, and the live comparable product already runs FL 23.4% vs a <10% target. Un-calibrated, gameable metrics would certify a product that misses the company's own quality bar (war-game F033). |
| **DEC-10** | Client chat, AI-message loop, Opportunity Radar (beyond one shadow card), e-invoice parsing, DATEV write automation, and real client data are **deferred to later, separately-gated phases**. | These add scope, authority, or data exposure the first proof does not need. Keeping them out makes the first phase provable and safe. |

Preserved from the MVP: the two-layer product shape (workbench + one deep job), human authority, synthetic-replay-first, no-client-contact-in-MVP, the two separate scorecards, and MET-D009's vendor-neutral intent (now realized via adapters).

---

## 4. The war-game result (what stress-tested these decisions)

We reran the full [war game](war_game.md) (v2.1) against the real SE-DIFM architecture: re-adjudicated all 32 prior scenarios, ran a fresh 10-front attack, and adversarially refuted every kill-shot claim.

- **Prior 32 scenarios (v2.1):** 2 survived, 22 unknown, 8 failed. The 8 `failed` are claim discipline (S011, S030), metrics/cost (S029), reviewer capacity at scale (S031), and one controlled-live integration prerequisite (S028) — all mapping to existing open questions.
- **Two provisional kill shots overruled by Product:** the greenfield-vs-vendor "two products" concern (→ vendor-agnostic via APIs, DEC-01) and the illegal-LLM-egress concern (→ PoC build out of scope, DEC-03).
- **Three real findings reclassified to their correct gates:** single-Steuerberater failover (F022 → controlled-live), filing authority / Untervollmacht (F024 → controlled-live), named security owner (F025 → real-data).
...
flowchart LR
    ...
    B --> C["2 kill shots overruled\n(vendor-agnostic; PoC out of scope)"]
...
The MVP's open questions O019–O029, plus what the rerun sharpened. Notion answered most of the *content*; the residue is decisions, named people, and pre-registered numbers.

### Close before build → replay (MET-G001) — design requirements + existing inputs

| Open | Question | Status after Notion + rerun |
| --- | --- | --- |
| O020 / O021 | Connected-source contract (via adapters); frozen case schema, rubric, gold pack. | **Substantially answered** (integration surface; Buchungsprofil; readiness rubric; match >85%). Residue: exact ~60-of-1200 SKR03/04 account list + SKR choice (Julian + Suat); enum values (OQ-1); prior-bookkeeping takeover. |
...
| O024 | Named sponsor, PM, advisors, gate participants. | **Answered** (E033): Markus (COO), Christine (PM), Claire (GTM), Massimo (Eng), Carolin (Ops), Ozan (Dashboard), Suat (StB), Carlo Piltz (DPO). Security owner deferred to O012. |
...
| --- | --- | --- |
| O025 / O011 (F022) | Reviewer absent/overloaded/late/rubber-stamping; failover for the single Steuerberater. | Partly answered (TA-Dashboard PRD rules) — but **no absence-failover for Freigabe**, and >15% alert sits below the live 23.4% FL floor. Controlled-live/scale prerequisite. |
...
| F031 | Recovery path for blocked cases (D019 no-client-contact tension). | Design requirement: owned human-ops queue or minimal non-AI nudge. |
...
| O027 (F031) | Post-handoff correction/harm window. | **Answered:** §347 AO Einspruchsfrist = 1 month; post-export re-file warning; 10yr audit; StBerG liability. Remediation workflow still under-specified. |
| F028 | DATEV sanctioned API/partner onboarding (4–12 wk). | Integration-sequencing item; schedule as a dated critical-path task. |
...
- **Controlled live / real client data:** additionally requires the reviewer failover (F022/O011), the filing authority (F024/O013), the named security owner + provider allowlist + AVV/DPAs (F025/O012/O019), and the DATEV partner onboarding (F028). These are real and non-negotiable at that gate — and correctly deferred to it.

This separation is the point: we can build and prove the MVP honestly on synthetic data now, while the go-live prerequisites are worked in parallel by their named owners.

---

## 7. The design requirements in plain words — build now vs iterate

The war game named six design requirements. Here they are in everyday language, and whether each must be designed in from day one or can be added iteratively. The short version: **build iteratively on top of three safety rails you install first.**

| ID | Plain-language name | What it means | Why it matters | Build now or iterate? |
| --- | --- | --- | --- | --- |
| **F026** | Govern the engine, don't rebuild it | The bookkeeping engine already does OCR, bookings, VAT, and matching. Our layer checks and packages its output; it does not redo the bookkeeping. | Rebuilding it adds a place for the AI to invent things, with no upside. | **Shape now** — it is the core architecture (adapter + review layer). Retrofitting later is a rewrite. |
| **F029** | Know how fresh the data is, fail safe when stale | Every fact carries a "last checked at" stamp. If it is too old, the system refuses to say "ready" instead of guessing. | The worst failure is calling a period "done" on yesterday's bank data. | **Shape now (minimal)** — fail-closed must be the default from the first run. Loosening later is easy; tightening later means re-auditing everything already shipped. |
| **F032** | Read documents, don't take orders from them | Accept the real formats (PDF/JPEG invoices, bank JSON, later e-invoices) or stop cleanly; and never let text inside a document act as an instruction to the AI. | A malicious or odd invoice must not be able to say "ignore the rules" and be obeyed. | **Shape now (minimal)** — a security boundary is cheap to design in, expensive and risky to bolt on after the pipeline exists. |
| **F030** | A strict "does this case belong here?" gate | Before any work starts, check the case fits the one allowed profile; anything outside → stop, don't process. | Without it, an out-of-scope case can quietly run and produce a confident but wrong result. | **Iterate** — start with a simple allow-list check, expand the exclusion rules case by case. |
| **F031** | A place for blocked cases to go | The most common stop ("a required invoice is missing") normally needs client contact, which the MVP does not do — so blocked cases route to an owned human queue with a turnaround. | Otherwise blocked cases pile up with no way to unblock. | **Iterate** — a basic "needs a human" tray is enough to start; refine the SLA and UX later. |
...
**The rule of thumb:** treat the three "shape now" items (F026, F029, F032) as *architectural defaults*, not features — "govern, don't rebuild", "fail closed unless proven fresh/in-scope/safe", and "document text is never an instruction" belong in the first lines of code. They cost almost nothing up front and are painful to add later. The other three (F030, F031, F033) can start rough and sharpen as you learn — the iterative path.
...
- **The governed review surface may add too little value over the engine alone.** If synthetic replay shows the verification + audit + routing layer does not measurably improve trusted completion or reduce Advisor effort, we narrow further or stop — that is what the gate is for (DEC-02, war-game H004).
...
- Output links: [MVP](agentic_mvp.md) | [war game](war_game.md) | [metrics](metrics_learning.md) | [GTM](gtm_scenario.md) | [todo before MVP](todo_before_mvp.md)
